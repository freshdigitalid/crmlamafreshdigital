...
function perfex_saas_get_tenant_quota_usage($tenant, $limits = [], $package = null)
{
    ...
    foreach ($limits as $limit) {
        ...
        $where_clause = hooks()->apply_filters('perfex_saas_tenant_quota_usage_query_where_clause', $where_clause, ['table' => $table, 'tenant' => $tenant, 'limit' => $limit, 'package' => $package, 'apply_period_limit' => $apply_period_limit]);

        // Get count for the active tenant from table and match against package.
        $queries[$limit] = "SELECT COUNT(*) as total FROM $table $where_clause";
    }

    $queries = hooks()->apply_filters('perfex_saas_tenant_quota_usage_queries', $queries, ['tenant' => $tenant, 'limits' => $limits, 'package' => $package, 'apply_period_limit' => $apply_period_limit]);
    ...
    $usage_list = hooks()->apply_filters('perfex_saas_tenant_quota_usage', $usage_list, ['tenant' => $tenant, 'limits' => $limits, 'package' => $package]);
