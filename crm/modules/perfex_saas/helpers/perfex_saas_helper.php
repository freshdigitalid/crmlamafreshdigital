...
require_once __DIR__ . '/perfex_saas_demo_instance_helper.php';
...
/**
 * Generate a tenant's cron URL with optional security key.
 *
 * @param object $tenant The tenant object
 * @param bool $setup_mode Whether to include setup query.
 * @param bool $secure Whether to include the APP_CRON_KEY in the URL.
 * @return string The generated cron URL.
 */
function perfex_saas_tenant_cron_url($tenant, bool $setup_mode = false, bool $secure = false): string
{
    $url_path = 'cron/index';
    if ($secure)
        $url_path .= (defined('APP_CRON_KEY') ? '/' . APP_CRON_KEY : '');

    if ($setup_mode)
        $url_path .= '?setup=1';

    return perfex_saas_tenant_base_url($tenant, $url_path, 'path');
}
