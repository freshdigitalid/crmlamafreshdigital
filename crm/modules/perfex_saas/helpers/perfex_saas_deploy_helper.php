...
function perfex_saas_deployer($company_id = '', $clientid = '', $limit = 5)
{
    ...
    foreach ($pending_companies as $company) {
        try {
            ...
            // Set to deploying
            $CI->perfex_saas_model->add_or_update('companies', ['id' => $company->id, 'status' => PERFEX_SAAS_STATUS_DEPLOYING]);
            ...
            $CI->perfex_saas_model->add_or_update('companies', ['id' => $company->id, 'status' => PERFEX_SAAS_STATUS_ACTIVE, 'status_note' => '']);
            ...
            if (!isset($cron_req) || empty($cron_req['response'])) {
                ...
                $cron_urls[] = perfex_saas_tenant_cron_url($company, true, false);
            }
        } catch (\Throwable $e) {

            // Rollback deployment and creation of the instance
            $error = ($company->name ?? '') . ' ' . ($company->slug ?? 'Unkown') . ' DC001:' . $e->getMessage();
            ...
            // Notify the admin about this error.
            try {
                // Notify supper admin
                $admin = perfex_saas_get_super_admin();
                $staffid = $admin->staffid;
                ...
                if (add_notification([
                    ...
                    'additional_data' => serialize([$error])
                ])) {
                    pusher_trigger_notification([$staffid]);
                }
            } catch (\Throwable $th) {
...
function perfex_saas_deploy_company($company, $silent_mode = false)
{
    try {
        ...
        if (!$silent_mode) {
            $error = ($company->name ?? '') . ' ' . ($company->slug ?? 'Unkown') . ' DC002:' . $th->getMessage();
...
function perfex_saas_remove_company($company, $silent_mode = false)
{
    ...
    try {
        ...
        $error = ($company->name ?? '') . ' ' . ($company->slug ?? 'Unkown') . ' RC001:' . $th->getMessage();
        ...
        if (!$silent_mode) {
            ...
            try {
                ...
                if (add_notification([
                    'touserid' => $staffid,
                    'description' => 'perfex_saas_not_customer_remove_instance_failed',
                    'link' => 'clients/client/' . $company->clientid,
                    'additional_data' => serialize([$error])
                ])) {
                    array_push($notifiedUsers, $staffid);
...
function perfex_saas_setup_dsn($dsn, $slug, $source_dsn = [], $return_queries_only = false)
{
    ...
    if (empty($show_create_queries)) {
        foreach ($master_tables as $table) {
            ...
            $sql = perfex_saas_master_sql_query_to_tenant(
                $sql,
                $slug,
                $table,
                $new_table,
    ...
    // The wholistic approach is neccessary for cross-referenced keys or table
    // that might not have been replaced for each individual table query
    ...
    foreach ($master_tables as $table) {
        ...
        // Rename table and foreign key
        $sql_commands_to_run = perfex_saas_master_sql_query_to_tenant(
            $sql_commands_to_run,
            $slug,
            $table,
            $new_table,
...
/**
 * Function to modify SQL queries for tenant-specific operations.
 * It changes table and foreign key references based on tenant-specific prefixes.
 *
 * @param string $sql The SQL query to be modified.
 * @param string $slug The tenant-specific identifier.
 * @param string $table The original table name with the master database prefix.
 * @param string $new_table The new table name with the tenant-specific prefix.
 * @param string $master_dbprefix The prefix used in the master database.
 * @param bool $strict_match Determine if table lookup should be safe and strict or loose
 * @return string The modified SQL query with tenant-specific prefixes.
 */
function perfex_saas_master_sql_query_to_tenant($sql, $slug, $table, $new_table, $master_dbprefix, $strict_match = true)
{
    // Remove the master database prefix from the table name to work with the tenant-specific table.
    $table_without_prefix =  str_ireplace($master_dbprefix, '', $table);
    $new_table_without_master_prefix = str_ireplace($master_dbprefix, '', $new_table);

    $prefix = $strict_match ? '`' : '';

    // Rename table and foreign key
    $key_constraint_search = [
        $prefix . $table,
        'KEY `' . $table_without_prefix,
        'CONSTRAINT `' . $table_without_prefix
    ];

    $key_constraint_search_replace_val = [
        $prefix . $new_table,
    ...
    if (!$strict_match) {
        if (
            stripos($sql, $key_constraint_search[1]) === false  &&
            stripos($sql, 'KEY `') !== false &&
            stripos($sql, 'KEY `' . $slug) === false
        ) {
            $key_constraint_search[] = 'KEY `';
            $key_constraint_search_replace_val[] = 'KEY `' . $slug;

            $key_constraint_search[] = 'CONSTRAINT `';
            $key_constraint_search_replace_val[] = 'CONSTRAINT `' . $slug;
        }
    }
...
function perfex_saas_seed_tables($dbprefix = '')
{
    ...
    // Require this to ensure leads_email_integration form will save as first row in this table is required by Perfex
    // Perfex treat this more like a setting row
    $lei_table = $dbprefix . 'leads_email_integration';
    if (!in_array($lei_table, $seed_tables))
        $seed_tables[] = $lei_table;
...
{
    // Allow custom control of the seeding process.
    $hooks = hooks()->apply_filters('perfex_saas_before_seeding_teannt', [
        'company' => $company,
        'dsn' => $dsn,
        'seed_source' => $seed_source,
        'seed_source_reference' => $seed_source_reference,
        'used_custom_seeding' => false
    ]);
    if (isset($hooks['used_custom_seeding']) && $hooks['used_custom_seeding'] === true) return true;
    ...
    if ($seed_source == PERFEX_SAAS_SEED_SOURCE_TENANT && !empty($seed_source_reference)) {
        ...
        if (!empty($seed_source_reference)) {

            // Attempt to seed image files
            perfex_saas_seed_tenant_media($company->slug, $seed_source_reference);
        }

        $seeding = perfex_saas_seed_tenant_from_dsn($company, $dsn, $source_dsn, $dbprefix);
    ...
    foreach ($seed_tables as $table) {
        ...
        foreach ($rows as $row) {
            ...
            if ($table === $dbprefix . 'options') {
                ...
                // Only apply this filter when source is super admin only.
                if ($source_is_super_admin && in_array($row['name'], $default_senstive_fields)) continue;
            }
...
function perfex_saas_seed_tenant_from_sql_files($company, $dsn, $folder_path = '')
{
    ...
    // Allow customizing the files
    $files = hooks()->apply_filters('perfex_saas_seed_tenant_files', $files, $company, $dsn);
    ...
    foreach ($files as $file) {
        ...
        if ($file_extension == 'sql') {
            ...
            $sqlStatements = $CI->sqlscriptparser->parse($file, function ($sqlString) use ($dbprefix, $tenant_dbprefix) {
...
function perfex_saas_seed_tenant_media($tenant_slug, $source_tenant_slug)
{
    $source_tenant_media_base_path = perfex_saas_tenant_upload_base_path(
        (object)['slug' => $source_tenant_slug]
    );

    $tenant_media_base_path = perfex_saas_tenant_upload_base_path(
        (object)['slug' => $tenant_slug]
    );

    try {
        return xcopy($source_tenant_media_base_path, $tenant_media_base_path);
    } catch (\Throwable $th) {
        log_message('error', $th->getMessage());
    }
}
