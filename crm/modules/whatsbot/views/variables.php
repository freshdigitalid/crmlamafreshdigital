<?php if (!empty($template)) { ?>
    ...
    <?php if (!empty($template['header_data_format']) && 'VIDEO' === $template['header_data_format']) { ?>
        <h4 class="tw-mt-0 tw-font-semibold tw-text-neutral-700"><?php echo _l('video'); ?></h4>
        <input type="hidden" id="maxFileSize" value="<?= $allowd_extension['video']['size']; ?>">
        ...
        <div class="view_campaign_image <?= (isset($campaign) && empty($campaign['filename'])) ? 'hide' : ''; ?>">
            <?php if (isset($campaign)) : ?>
                <div class="row">
                    <div class="col-md-9">
                        <input type="hidden" id="image_url" value="<?= (!empty($campaign['filename'])) ? base_url(get_upload_path_by_type($campaign['is_bot'] == '1' ? 'template' : 'campaign') . $campaign['filename']) : ''; ?>">
                        <video controls src="<?= base_url(get_upload_path_by_type($campaign['is_bot'] == '1' ? 'template' : 'campaign') . $campaign['filename']); ?>" class="img img-responsive" height="70%" width="70%">
                    </div>
                    <div class="col-md-3 text-right">
                        <a href="<?= admin_url(WHATSBOT_MODULE . '/campaigns/delete_campaign_files/' . $campaign['id']); ?>" class="_delete"><i class="fa fa-remove text-danger"></i></a>
                    </div>
        ...
        <div class="campaign_image <?= (isset($campaign) && !empty($campaign['filename'])) ? 'hide' : ''; ?>">
            <label for="video" class="control-label">
                <i class="fa-regular fa-circle-question pull-left tw-mt-0.5 tw-mr-1" data-toggle="tooltip" data-title="<?= _l('maximum_file_size_should_be') . $allowd_extension['video']['size'] . ' MB'; ?>"></i>
                <?= _l('select_video'); ?>
                <small class="text-muted">( <?= _l('allowed_file_types') . $allowd_extension['video']['extension']; ?> )</small>
            </label>
            <input type="file" name="video" id="video" accept="<?= $allowd_extension['video']['extension']; ?>" class="form-control header_image">
        </div>
        <hr>
    <?php } ?>
